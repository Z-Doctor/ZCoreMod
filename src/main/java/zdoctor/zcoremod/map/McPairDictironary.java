				...
				if (value instanceof McObfClass) {
					...
					if (entryFile != null) {
						try {
							...
							entryStream.write(String.format(Config.ENTRYSTART,
									name.replaceAll("/", ".").substring(0, name.lastIndexOf('/')),
									name.substring(name.lastIndexOf('/') + 1)).getBytes());
							entryStream.write(Config.GETKEY.getBytes());
							entryStream.write(
									String.format(Config.GETCLASSNAME, value.getObf(), value.getDeobf()).getBytes());
							entryStream.write(String
									.format(Config.GETNAME,
											value.getObf().substring(value.getObf().lastIndexOf('/') + 1),
											value.getDeobf().substring(value.getDeobf().lastIndexOf('/') + 1))
									.getBytes());
							entryStream.write(String.format(Config.GETDESCRIPTOR, "L" + value.getObf() + ";",
									"L" + value.getDeobf() + ";").getBytes());
...
//								name.replaceAll("/", ".").substring(0, name.lastIndexOf('/')),
//								name.substring(name.lastIndexOf('/') + 1)).getBytes());
//						entryStream.write(String.format(Config.GETKEY, PrimitiveUtil.toString(key)).getBytes());
//						entryStream.write(String.format(Config.GETKEY, value.getClassName()).getBytes());
//						files.register(name, entryStream);
//					}
...
//						System.out.println(name);
						...
						if (value instanceof McObfMethod) {
							entryStream.write('\t');
							entryStream.write(String.format(Config.FUNCFORMATCOMMENT, value.getVariableName(),
									Arrays.toString(value.getDescriptors())).getBytes());
						} else if (value instanceof McObfField) {
							entryStream.write('\t');
							entryStream.write(
									String.format(Config.FIELDFORMATCOMMENT, value.getVariableName()).getBytes());
						}
//
						entryStream.write('\t');
						if (value instanceof McObfField) {
							entryStream.write(String.format(Config.ENTRYFORMAT,
									PrimitiveUtil.toString(key).replaceAll(".*\\.", ""), PrimitiveUtil.toString(key))
									.getBytes());
...
//							entryStream.write(String
//									.format(Config.ENTRYFORMAT, value.getVariableName(), PrimitiveUtil.toString(key))
//									.getBytes());
					}
				} catch (IOException e) {
					e.printStackTrace();
